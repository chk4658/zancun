...
<script>
  ...
  function setCopy() {
    ...
    for (let i = 0; i < this.projectMilestones.length; i++) {
      ...
      let len = this.projectMilestones[i].taskList.length;
      if (len + currentLength <= num) {
        if (!this.projectMilestones[i].collapsed) {
          len = 0;
        } else {
          len = this.projectMilestones[i].taskList.length;
        }
        currentLength = currentLength + len;
        this.copyPM.push(this.projectMilestones[i]);
        continue
      } else {
        if (!this.projectMilestones[i].collapsed) {
          len = 0;
        ...
        const splice = num - currentLength;
        const aa = JSON.parse(JSON.stringify(this.projectMilestones[i]))
        aa.taskList.splice(splice, aa.taskList.length + 1);
        this.copyPM.push(aa);
  ...
  //   console.log(this.currentPage)
  //   const parentTask = []
  //   this.projectMilestones.forEach(pm => {
  //     if (pm.collapsed) {
  //       parentTask.push(...pm.taskList)
  //     }
  //   })
  //   const range = [(this.pageSize * (this.currentPage - 1)) + 1, this.pageSize * this.currentPage];
  //   if (parentTask.length < range[0]) {
  //     return false
  //   }
  //   if (parentTask.length <= range[1]) {
  //     range[1] = parentTask.length
  //   }
  //
  //   let firstIndex = 0;
  //   let lastIndex = 0;
  //   this.projectMilestones.forEach((pm, index) => {
  //     if (pm.taskList.indexOf(parentTask[range[0] - 1]) !== -1) {
  //       firstIndex = index
  //     }
  //     if (pm.taskList.indexOf(parentTask[range[1] - 1]) !== -1) {
  //       lastIndex = index
  //     }
  //   })
  //   if (firstIndex === lastIndex) {
  //     if (this.copyPM.length === 0 || (this.projectMilestones[firstIndex].taskList !== undefined && this.projectMilestones[firstIndex].taskList.length === 0)) {
  //       const splice = this.pageSize;
  //       const aa = JSON.parse(JSON.stringify(this.projectMilestones[firstIndex]))
  //       aa.taskList.splice(splice, aa.taskList.length + 1);
  //       this.copyPM.push(aa);
  //     } else {
  //       const aa = JSON.parse(JSON.stringify(this.projectMilestones[firstIndex]))
  //       const bb = aa.taskList.slice(aa.taskList.indexOf(parentTask[range[0] - 1]), aa.taskList.indexOf(parentTask[range[1] - 1]) + 1);
  //       this.copyPM[this.copyPM.length - 1].taskList.push(...bb)
  //     }
  //
  //   } else {
  ...
  //         } else {
  //           const aa = JSON.parse(JSON.stringify(this.projectMilestones[firstIndex]))
  //           const bb = aa.taskList.slice(aa.taskList.indexOf(parentTask[range[0] - 1]));
  //           this.copyPM[this.copyPM.length - 1].taskList.push(...bb)
  //         }
  //         continue
  //       }
  //       if (i === lastIndex) {
  //         const aa = JSON.parse(JSON.stringify(this.projectMilestones[lastIndex]))
  //         const splice = aa.taskList.indexOf(parentTask[range[1] - 1]) + 1
  //         aa.taskList.splice(splice, aa.taskList.length + 1);
  //         this.copyPM.push(aa);
  //       }
  //     }
  ...
  //       const splice = num - currentLength;
  //
  //       if (splice - this.pageSize > 0) {
  //         const aa = JSON.parse(JSON.stringify(this.projectMilestones[i]))
  //         const temTask = aa.taskList.slice(splice - this.pageSize, splice + 1)
  //         aa.taskList.splice(splice - this.pageSize, aa.taskList.length + 1);
  //         this.copyPM.push(aa);
  //         this.copyPM.filter(cp => cp.projectMilestone.id === this.projectMilestones[i].projectMilestone.id)[0].taskList.push(...temTask)
  //       } else {
  //         const aa = JSON.parse(JSON.stringify(this.projectMilestones[i]))
  //         aa.taskList.splice(splice, aa.taskList.length + 1);
  //         this.copyPM.push(aa);
  ...
  export default {
    ...
    computed: {
      ...
      noMore() {
        return this.copyPM.length === this.projectMilestones.length
          && (this.copyPM[this.copyPM.length - 1] !== undefined
            && this.copyPM[this.copyPM.length - 1].taskList.length === this.projectMilestones[this.projectMilestones.length - 1].taskList.length);
      },
      disabled() {
        return this.loading || this.noMore || this.projectMilestones.length === 0
      }

<template>
  <div>
    ...
    <el-dialog
      ...
      <task-edit-child :task="taskChild"
                       :taskAttr="taskAttr"
                       @save="closeChildTask"
                       @re="refreshChildDelivery"
                       @update="listChildTask"
                       :reBeginAuthority="reBeginAuthority"
                       :authority="authority"
                       :rolePlus="rolePlus"></task-edit-child>
    </el-dialog>
...
<script>
  ...
  import TaskEditChild from "./TaskEditChild";
  ...
  async function getRoles(data) {
    ...
    if (this.flag) {
      ...
      if (result.code === 1200) {
        ...
        // this.taskData.reviewRoleName = data[0];
        // this.taskData.confirmRoleName = this.taskData.confirmProjectRoleId === null ? [] : this.taskData.confirmProjectRole.roleName;
        //
        //
        // const result1 = await _listByTaskId(this.taskData.id);
        // if (result1.code === 1200) {
        //   this.taskData.reviewProjectRoleId = result1.data.task.reviewProjectRoleId;
        //   this.taskData.reviewProjectRole = {
        //     roleName: data[0],
        //     id: result1.data.task.reviewProjectRoleId
        //   };
        //   if (data.length === 0) {
        //
        //     this.taskData.reviewUser = null
        //   } else {
        //     const us = await _listProjectRoleUser({projectRoleId: this.taskData.reviewProjectRoleId});
        //
        //
        //     if (us.code === 1200 && us.data.sysUserList.length !== 0) {
        //       this.taskData.reviewUser = us.data.sysUserList[0];
        //     } else {
        //       this.taskData.reviewUser = null
        //     }
        //   }
      ...
      if (result.code === 1200) {
        ...
        // this.taskData.reviewRoleName = this.taskData.reviewProjectRoleId === null ? [] : this.taskData.reviewProjectRole.roleName;
        // this.taskData.confirmRoleName = data[0];
        //
        // const result2 = await _listByTaskId(this.taskData.id);
        // if (result2.code === 1200) {
        //   this.taskData.confirmProjectRoleId = result2.data.task.confirmProjectRoleId;
        //   this.taskData.confirmProjectRole = {
        //     roleName: data[0],
        //     id: result2.data.task.confirmProjectRoleId
        //   };
        //
        //   if (data.length === 0) {
        //
        //     this.taskData.confirmUser = null
        //
        //   } else {
        //     const er = await _listProjectRoleUser({projectRoleId: this.taskData.reviewProjectRoleId});
        //
        //     if (er.code === 1200 && er.data.sysUserList.length !== 0) {
        //       this.taskData.confirmUser = er.data.sysUserList[0];
        //     } else {
        //       this.taskData.confirmUser = null
        //     }
        //   }
  ...
  async function getOwnersId(data) {
    if (data !== null && data.length > 0) {
      ...
      //   }
      // } else {
      //   this.taskData.confirmUser = {
      //     id: data[0].id,
      //     realName: data[0].realName,
      //   };
      //   const result = await _saveOrUpdateRoleUser({
      //     projectRoleId: this.taskData.confirmProjectRoleId,
      //     userId: data[0].id,
      //   });
      //   if (result.code === 1200) {
      //     this.$message({
  ...
  function refreshChildDelivery() {

    this.$emit('re')
  }

  async function listChildTask() {
    const result = await _getToPut(this.taskData.projectId, '', this, this.types, this.person);
    const childs = result.childTaskList;

    if (this.currentTaskId !== '' && childs.filter(r => r.id === this.currentChildTaskId).length !== 0) {
      this.taskChild = childs.filter(r => r.id === this.currentChildTaskId)[0]
    }
  }
